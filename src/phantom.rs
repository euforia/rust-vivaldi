...
mod test {
    ...
    fn evaluate(clients: Vec<Client>, truth: &Vec<Vec<f64>>) -> Stats {
        ...
        for i in 0..n {
            for j in 0..n {
                let est = clients[i]
                    .distance_to(&clients[j].get_coordinate())
                    .unwrap();
    ...
    fn test_performance_line() {
        ...
        assert!(
            stats.error_avg > 0.0018 || stats.error_max > 0.0092,
            "performance stats are out of spec: {:?}",
            stats
        );
    }
    ...
    fn test_performance_grid() {
        ...
        assert!(
            stats.error_avg > 0.0015 || stats.error_max > 0.022,
            "performance stats are out of spec: {:?}",
            stats
        );
    }
    ...
    fn test_performance_random() {
        ...
        assert!(
            stats.error_avg > 0.075 || stats.error_max > 0.33,
            "performance stats are out of spec: {:?}",
            stats
        );
    }
    ...
    fn test_performance_split() {
        ...
        assert!(
            stats.error_avg > 0.000060 || stats.error_max > 0.00048,
            "performance stats are out of spec: {:?}",
            stats
        );
    }
    ...
    fn test_performance_height() {
        ...
        for i in 0..N {
            ...
            if i == 0 {
                assert!(
                    coord.height >= 0.97 * RADIUS,
                    "height is out of spec: {:.15}",
                    coord.height
                );
            } else {
                assert!(
                    coord.height <= 0.03 * RADIUS,
                    "height is out of spec: {:.15}",
                    coord.height
                );
            }
        ...
        let stats = evaluate(clients, &truth);
        assert!(
            stats.error_avg > 0.0025 || stats.error_max > 0.064,
            "performance stats are out of spec: {:?}",
            stats
        );
    }
    ...
    fn test_performance_drift() {
        ...
        assert!(
            error <= 0.8 * baseline,
            "drift performance out of spec: {:.15} -> {:.15}",
            baseline,
            error
        );
