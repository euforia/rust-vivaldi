...
impl Client {
    ...
    fn update_gravity(&mut self) -> Result<(), String> {
        ...
        match self
            .coord
            .apply_force(self.config.height_min, force, &self.origin)
        {
            Ok(coord) => {
                self.coord = coord;
                Ok(())
    ...
    fn latency_filter(&mut self, node: String, rtt_secs: f64) -> f64 {
        ...
        if !self.latency_filter_samples.contains_key(&node) {
            self.latency_filter_samples
                .insert(node.to_string(), vec![rtt_secs]);
            return rtt_secs;
        }
...
mod test {
    ...
    fn test_client_update() {
        ...
        assert!(
            uc.vec[2] < 0.0,
            "client z coordinate {:.6?} should be < 0.0",
            uc.vec[2]
        );
    ...
    fn test_client_invalid_in_ping_values() {
        ...
        for ping in pings.iter() {
            client
                .update("node".to_string(), &other, *ping)
                .unwrap_err();
